...
from keras.applications import DenseNet169, MobileNet
...
class MuraGenerator(Sequence):
    def __init__(self, paths_studies, batch_size, weights, augment=True):
        self.bs = batch_size
...
if __name__ == "__main__":
    ...
    studies_path = np.asarray(pd.read_csv(
        args.train_path, delimiter=',', header=None))
    ...
    if args.stage < 3:
        ...
        checkpoint_path = 'models/stage-{}-'.format(
            args.stage) + 'modeldef-e{epoch:03d}.hdf5'
        csvlogger = CSVLogger(
            'logs/defstage-{}-{}.log'.format(args.stage, args.section), append=args.resume)
        checkpointer = ModelCheckpoint(checkpoint_path, save_best_only=False,
                                       save_weights_only=True)
        reduce_lr = ReduceLROnPlateau(monitor='val_binary_accuracy', factor=0.1,
        ...
        for study in studies_path:
            ...
            img_paths = glob(str(study[0]) + '*.png')
            images = []
            for img_path in img_paths:
                img = image.load_img(img_path, color_mode='rgb',
                                     target_size=(320, 320))
        ...
        print("\tLoss (weights): ", log_loss(
            y_true, y_pred, sample_weight=sample_w))
        print("\tAccuracy: ", accuracy_score(y_true, y_pred_bin))
        print("\tAccuracy (weights): ", accuracy_score(
            y_true, y_pred_bin, sample_weight=sample_w))
        print("\tRecall: ", recall_score(y_true, y_pred_bin))
        print("\tRecall (weights): ", recall_score(
            y_true, y_pred_bin, sample_weight=sample_w))
        print("\tPrecision: ", precision_score(y_true, y_pred_bin))
        print("\tPrecision (weights): ", precision_score(
            y_true, y_pred_bin, sample_weight=sample_w))
        print("\tAUC: ", roc_auc_score(y_true, y_pred))
        print("\tAUC (weights): ", roc_auc_score(
            y_true, y_pred, sample_weight=sample_w))
